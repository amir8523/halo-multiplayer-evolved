...
// ForgeEvolved: tools/probe/LoaderTest.cpp
//
// Smoke test for the built mod DLL.
//
// Loads ForgeEvolved.dll exactly as the game's loader proxy does, waits for its
...
int main(int argc, char** argv) {
    ...
    Print("loading ForgeEvolved.dll ...");
    const HMODULE module =
        ::LoadLibraryExW(L"ForgeEvolved.dll", nullptr, LOAD_WITH_ALTERED_SEARCH_PATH);
    if (module == nullptr) {
    ...
    const auto version = Resolve<PFN_Version>(module, "FE_Version");
    const auto ready   = Resolve<PFN_IsReady>(module, "FE_IsReady");
    const auto phase   = Resolve<PFN_Phase>(module, "FE_Phase");
    const auto host    = Resolve<PFN_Host>(module, "FE_HostSession");

    if (version == nullptr || ready == nullptr || phase == nullptr || host == nullptr) {
        Print("FAIL: one or more exports did not resolve");
        return 1;
    }
    Print(std::string("FE_Version() = ") + version());
    ...
    for (int second = 1; second <= wait_seconds; ++second) {
        ::Sleep(1000);
        Print("t+" + std::to_string(second) + "s  FE_IsReady=" + std::to_string(ready()) +
              "  FE_Phase=" + phase());
    }

    // Hosting must be refused here: the engine binding cannot resolve in a process
    // that has no engine. A negative value is the negated fe::ErrorCode, and
    ...
    Print("FE_HostSession(...) = " + std::to_string(host_result) +
    ...
    if (const auto lobby_test = Resolve<PFN_LobbyTest>(module, "FE_LobbySelfTest");
        lobby_test != nullptr) {
        Print("");
        Print("running lobby self test (creates a real Steam lobby) ...");
        const int lobby_result = lobby_test(20);
        Print("FE_LobbySelfTest(20) = " + std::to_string(lobby_result) +
              (lobby_result == 0 ? "  PASSED" : "  FAILED, see the log"));
    } else {
        Print("FE_LobbySelfTest not exported by this build");
    }

    Print("");
    Print("done. Inspect ForgeEvolved\\ForgeEvolved.log for the startup record.");
